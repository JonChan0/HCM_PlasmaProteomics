...
timelag_plotter <- function(input_tb, metric, metric_name, pp_of_interest, output_folder, suffix=''){
  ...
  p <- p + facet_wrap(~num_instances_sig, scales = "fixed")+
  ...
  ggsave(str_c(output_folder, 'summary_',metric, '_lineplot',suffix,'.png'), p, dpi=600, width = 12, height = 6)
  
}

timelag_plotter(lag_results, 'B', 'Beta',fdr5_proteins_all_lag, 'OUTPUT/UKB/PLOTS/1_Case_vs_Control/2_Limma/0_TimeInstancing/HCM/rcmm_cov/allpanels/')
...
timelag_plotter(lag_results, 'B', 'Beta',fdr5_proteins_all_lag[!fdr5_proteins_all_lag %in% c('ntprobnp','nppb')], 'OUTPUT/UKB/PLOTS/1_Case_vs_Control/2_Limma/0_TimeInstancing/HCM/rcmm_cov/allpanels/', suffix='_wo_NTproBNP_NPPB')
```
...
fdr5_Cox_pp_all_lag <- as.character(unique(filter(cox_lag_results, fdr5_pass_all_lags==T)$pp)) #186 plasma proteins are FDR5-sig. in at least one time instance
