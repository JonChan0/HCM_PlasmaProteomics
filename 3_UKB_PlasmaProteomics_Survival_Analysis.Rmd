...
#Check the distribution of NTproBNP and HRC 
print(hist(ukb_pp_hcm_cov2$ntprobnp))
print(hist(ukb_pp_hcm_cov2$hrc))

print(mean(ukb_pp_hcm_cov2$ntprobnp, na.rm=T))
print(mean(ukb_pp_hcm_cov2$hrc, na.rm=T))

print(sd(ukb_pp_hcm_cov2$ntprobnp, na.rm=T))
print(sd(ukb_pp_hcm_cov2$hrc, na.rm=T))
...
#' @export
#' @examples
#' if (require(dplyr) && require(tibble) && require(matrixStats) && require(purrr)) {
#'   # Define the add_composite_outcome_filtered function (from previous request)
#'   # [User would need to have the add_composite_outcome_filtered function defined here]
#'   # For brevity, I'll assume it's defined and use its example output structure.
#'
#'   set.seed(123)
#'   example_df_for_summary <- tibble::tibble(
#'     id = c(1, 4, 6, 8, 9, 10), # Assuming filtered IDs from previous example
#'     age_at_assessment = c(45, 59, 65, 68, 70, 71),
#'     enrollment_age = c(40, 41, 50, 52, 55, 58), # For TSS example
#'     arrest_age = c(NA, 60, NA, NA, NA, 65), # Note: ID 10 arrest_age=65 is pre-baseline for status
#'     scd_age = c(50, 62, 68, NA, 72, NA),
#'     icd_age = c(NA, NA, NA, 71, NA, NA),
#'     censoring_age = c(75, 61, 72, 78, 75, 77),
#'     # Output from add_composite_outcome_filtered (Example 1 simplified)
#'     vacomp_status = c(1, 1, 1, 1, 1, 0), # ID 10 has status 0
#'     vacomp_age = c(50, 60, 68, 71, 72, 77) # Age for ID 10 is censoring age
#'   )
#'   
#'   va_cols_for_summary <- c("arrest_age", "scd_age", "icd_age")
#'
#'   # Example 1: Summarize based on absolute age (tss = FALSE)
#'   summary_abs <- summarize_event_components(
#'     data = example_df_for_summary,
#'     outcome_cols = va_cols_for_summary,
#'     status_col_name = "vacomp_status",
#'     age_col_name = "vacomp_age",
#'     tss = FALSE
#'   )
#'   print("Summary of first events (absolute age):")
#'   print(summary_abs)
#'   # Expected: arrest_age: 1, scd_age: 3, icd_age: 1
#'
#'   # Prepare data similar to TSS output for Example 2
#'   example_df_for_summary_tss <- tibble::tibble(
#'     id = c(1, 4, 6, 8, 9, 10),
#'     age_at_assessment = c(45, 59, 65, 68, 70, 71),
#'     enrollment_age = c(40, 41, 50, 52, 55, 58),
#'     arrest_age = c(NA, 60, NA, NA, NA, 65),
#'     scd_age = c(50, 62, 68, NA, 72, NA),
#'     icd_age = c(NA, NA, NA, 71, NA, NA),
#'     censoring_age = c(75, 61, 72, 78, 75, 77),
#'     vacomp_status_tss = c(1, 1, 1, 1, 1, 0),
#'     # vacomp_time_since_enroll: event_age - enrollment_age or censoring_age - enrollment_age
#'     vacomp_time_since_enroll = c(50-40, 60-41, 68-50, 71-52, 72-55, 77-58)
#'   )
#'
#'   # Example 2: Summarize based on time-since-start (tss = TRUE)
#'   summary_tss <- summarize_event_components(
#'     data = example_df_for_summary_tss,
#'     outcome_cols = va_cols_for_summary,
#'     status_col_name = "vacomp_status_tss",
#'     age_col_name = "vacomp_time_since_enroll",
#'     tss = TRUE,
#'     start_age_col = "enrollment_age"
#'   )
#'   print("Summary of first events (time-since-start):")
#'   print(summary_tss)
#'   # Expected: arrest_age: 1, scd_age: 3, icd_age: 1
#' }
...
```{r}
pp_of_interest <- c('ntprobnp','hrc','ace2')
...
km_survival_curv_plotter <- function(input_tb, categorical_var, status, outcome_var, plot_output_folder){
  ...
  for (type in plot_types){
    ...
    plot_obj <- ggsurvplot(
      ...
      ggtheme = theme_classic() # Added a theme for better aesthetics, optional
    )
...
I will limit the plasma proteins to a select few e.g
...
multivariate_cox <- function(input_tb, marginal_predictor, covariates, status, outcome_var, baseline_age='age_attend_i0', outcome_levels=c('VA','HF','OVERALL'), return_predictor_results_only=T, return_model=F){ #Return only the coefficient and p-values for the marginal_predictor
...
multivariate_cox_results <- c(
  map(pp, ~multivariate_cox(ukb_pp_hcm_outcome_tbs[[1]],.,covariates,statuses[1],surv_ages[1])),
  map(pp, ~multivariate_cox(ukb_pp_hcm_outcome_tbs[[2]],.,covariates,statuses[2],surv_ages[2])),
  map(pp, ~multivariate_cox(ukb_pp_hcm_outcome_tbs[[3]],.,covariates,statuses[3],surv_ages[3]))
) %>%
  bind_rows() %>%
...
multivariate_cox_models <- c(
  map(pp, ~multivariate_cox(ukb_pp_hcm_outcome_tbs[[1]],.,covariates,statuses[1],surv_ages[1], return_model=T)),
  map(pp, ~multivariate_cox(ukb_pp_hcm_outcome_tbs[[2]],.,covariates,statuses[2],surv_ages[2], return_model=T)),
  map(pp, ~multivariate_cox(ukb_pp_hcm_outcome_tbs[[3]],.,covariates,statuses[3],surv_ages[3], return_model=T))
)
...
multivariate_coxph_plotter <- function(multivariate_results, plot_output_path, output_name, selected_vars = c('ntprobnp','hrc','ace2'),FDR=5, xlim=c(0,10), ylim=c(0,6), pp_scaling =F){
...
multivariate_cox_results <- multivariate_cox_results %>%
  mutate(predictor = factor(predictor, levels=c('NTPROBNP','HRC', 'ACE2')))

multivariate_coxph_plotter(multivariate_cox_results,path, 'ntprobnp_alloutcomes_fdr5', selected_vars=c('ntprobnp'), pp_scaling = pp_scaling,xlim=c(0,7))
multivariate_coxph_plotter(multivariate_cox_results,path, 'hrc_alloutcomes_fdr5', selected_vars=c('hrc'), pp_scaling = pp_scaling,xlim=c(0,7))
multivariate_coxph_plotter(multivariate_cox_results,path, 'allpp_alloutcomes_fdr5', pp_scaling = pp_scaling,xlim=c(0,7))
multivariate_coxph_plotter(multivariate_cox_results, path, 'allpp_alloutcomes_bonferroni', FDR=F, xlim=c(0,7), pp_scaling = pp_scaling)
...
td_covar_ntprobnp_overallcomp_model <- multivariate_cox_models[[5]]
td_covar_hrc_overallcomp_model <- multivariate_cox_models[[6]]
...
#For VACOMP
walk2(multivariate_cox_models[1:3],pp, ~plot_martingale_linearity(ukb_pp_hcm_outcome_tbs[[1]],..1,..2,'VACOMP'))
walk2(multivariate_cox_models[4:6],pp, ~plot_martingale_linearity(ukb_pp_hcm_outcome_tbs[[2]],..1,..2,'HFCOMP'))
walk2(multivariate_cox_models[7:9],pp, ~plot_martingale_linearity(ukb_pp_hcm_outcome_tbs[[3]],..1,..2,'OVERALLCOMP'))
...
joint_cox_results <- bind_rows(
  multivariate_cox(ukb_tb,pp,covariates,statuses[1],surv_ages[1], return_predictor_results_only = F),
  multivariate_cox(ukb_tb,pp,covariates,statuses[2],surv_ages[2], return_predictor_results_only = F),
  multivariate_cox(ukb_tb,pp,covariates,statuses[3],surv_ages[3], return_predictor_results_only = F)
) %>%
  bind_rows() %>%
  filter(!is.na(p.value)) %>%
  filter(var %in% pp)

joint_cox_model_overallcomp <- multivariate_cox(ukb_tb,pp,covariates,statuses[3],surv_ages[3], return_model=T)
ntprobnp_model_overallcomp <- multivariate_cox(filter(ukb_tb, !is.na(hrc)),'ntprobnp',covariates,statuses[3],surv_ages[3], return_model=T) #Need to fit to the same dataset size for ANOVA
...
#'   (e.g., added via tmerge with कुत्ते() beforehand).
...
#' #                            bmi_at_recruitment = कुत्ते(bmi_at_recruitment),
#' #                            sex = कुत्ते(sex))
...
#' }
#'
#' @seealso \code{\link[survival]{tmerge}}, \code{\link[survival]{tdc}}, \code{\link[survival]{कुत्ते}}
...
add_baseline_as_tdc <- function(td_data,
  ...
  if (!(covariate_col %in% names(td_data))) {
     stop("td_data must contain column: ", covariate_col,
          ". Ensure it was added (e.g., via tmerge with कुत्ते()).")
  }
