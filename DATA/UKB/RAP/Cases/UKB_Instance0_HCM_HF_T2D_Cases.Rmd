...
bcollection_date <- read_tsv('2024_02_09_allUKB_BloodCollection_Time.tsv')%>%
...
hf_tb <- select(hf_diag_tb, ID=eid, firstocc_hf=p131354) %>%
  ...
  left_join(bcollection_date, by='ID')
...
```{r warnings=F}
i0_extractor <- function(input_tb, output_name, threshold_date='bcollection_date_i0',output_folder='../../PROCESSED/'){
  
  output_tb <- input_tb %>%
    mutate(i0_case= ifelse(min_diag_date <= eval(parse(text=threshold_date)), T,F))
...
fiveyear_extractor <- function(input_tb, output_name, threshold_date='bcollection_date_i0', output_folder='../../PROCESSED/'){
  
  output_tb <- input_tb %>%
    mutate(fiveyear_case = ifelse(time_length(min_diag_date - eval(parse(text=threshold_date)), unit='year') <= 5 |  #Assign 5Y status if their minimum date of diagnosis was within 5 years after attendance. NB This includes i0 cases because they have a time_length of negative because min_diag_Date < date_attend_i0
                                    time_length(death_date - eval(parse(text=threshold_date)), unit='year') <=5, T,F)) #OR if they had mention of death with ICD10 code of interest & died within 5 years of attendance
...
This outputs the total tb of when total HCM cases were diagnosed as a minimum of selfrep; hospital; death cause dates.

```{r}

hcm_tb2 <- death_date_extractor(hcm_tb,death_dates, death_cause_record, 'I42.1|I42.2') %>%
  rowwise() %>%
  mutate(min_diag_date2 = min(min_diag_date, death_date, na.rm=T))%>%
  mutate(min_diag_date2 = as_date(ifelse(is.infinite(min_diag_date2), NA, min_diag_date2)))

saveRDS(hcm_tb2, '../../PROCESSED/3_TOTAL/ukb_diagnosis_dates_minimum.rds')
