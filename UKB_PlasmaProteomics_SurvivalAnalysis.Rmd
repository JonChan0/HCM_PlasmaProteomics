...
pp_of_interest <- c('ntprobnp','hrc','ace2','tnni3','angpt2')
...
for (pp in pp_of_interest){
  ukb_tb <- contvar_binner(ukb_tb, pp, signif(quantile(ukb_tb[pp],probs=0.75,na.rm=T),3))
}
rm(pp)
```
...
walk(catvars_of_interest, ~km_survival_curv_plotter(ukb_tb, ., 'overallcomp_status','overallcomp_age',plotpaths[3])) #Using top 25% vs. bottom 75%

#Run for UKB - HF Composite
walk(catvars_of_interest, ~km_survival_curv_plotter(ukb_tb, ., 'hfcomp_status','hfcomp_age',plotpaths[2])) 

#Run for UKB - VA Composite
walk(catvars_of_interest, ~km_survival_curv_plotter(ukb_tb, ., 'vacomp_status','vacomp_age',plotpaths[1])) 
```
# Cox Regression
...
multivariate_coxph_plotter(multivariate_cox_results,path, 'ntprobnp_alloutcomes_fdr5', selected_vars=c('ntprobnp'), pp_scaling = pp_scaling,xlim=c(0,6))
multivariate_coxph_plotter(multivariate_cox_results,path, 'hrc_alloutcomes_fdr5', selected_vars=c('hrc'), pp_scaling = pp_scaling,xlim=c(0,6))
multivariate_coxph_plotter(multivariate_cox_results,path, 'allpp_alloutcomes_fdr5', pp_scaling = pp_scaling,xlim=c(0,6))
multivariate_coxph_plotter(multivariate_cox_results, path, 'allpp_alloutcomes_bonferroni', FDR=F, xlim=c(0,6), pp_scaling = pp_scaling)
