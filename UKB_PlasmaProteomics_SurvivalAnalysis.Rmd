...
multivariate_coxph_plotter <- function(multivariate_results, plot_output_path, output_name, selected_vars = c('ntprobnp','hrc'),FDR=5, xlim=c(0,8), ylim=c(0,7), pp_scaling =F){
...
multivariate_cox_results <- multivariate_cox_results %>%
  mutate(predictor = factor(predictor, levels=c('NTPROBNP','HRC')))
...
multivariate_coxph_plotter(multivariate_cox_results,path, 'ntprobnp_hrc_alloutcomes_fdr5', pp_scaling = pp_scaling)
multivariate_coxph_plotter(multivariate_cox_results, path, 'ntprobnp_hrc_alloutcomes_bonferroni', FDR=F, ylim=c(0,8), pp_scaling = pp_scaling)
