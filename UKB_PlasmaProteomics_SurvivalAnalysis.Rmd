...
# km_survival_curv_plotter(ukb_tb,ace2_categorical,overallcomp_status,overallcomp_age, plotpaths[3]) #Using top 25% vs. bottom 75% of HRC
...
# km_survival_curv_plotter(ukb_tb,ace2_categorical,hfcomp_status,hfcomp_age, plotpaths[2]) #Using top 25% vs. bottom 75% of HRC
...
# km_survival_curv_plotter(ukb_tb,ace2_categorical,vacomp_status,vacomp_age, plotpaths[1]) #Using top 25% vs. bottom 75% of HRC
```
# Cox Regression
...
multivariate_coxph_plotter(multivariate_cox_results, str_c('./OUTPUT/UKB/PLOTS/4_SURVIVAL/2_COXPH/',instance,'/'), 'ntprobnp_hrc_alloutcomes_fdr5')
multivariate_coxph_plotter(multivariate_cox_results, str_c('./OUTPUT/UKB/PLOTS/4_SURVIVAL/2_COXPH/',instance,'/'), 'ntprobnp_hrc_alloutcomes_bonferroni', FDR=F)
# 
# multivariate_coxph_plotter(multivariate_cox_results, str_c('./OUTPUT/UKB/PLOTS/4_SURVIVAL/2_COXPH/',instance,'/'), 'ntprobnp_ace2_alloutcomes_fdr5')
# multivariate_coxph_plotter(multivariate_cox_results, str_c('./OUTPUT/UKB/PLOTS/4_SURVIVAL/2_COXPH/',instance,'/'), 'ntprobnp_ace2_alloutcomes_bonferroni', FDR=F)
```
### Cox Assumptions
...
ph_assumption_tester <- function(model, plot_output_path, plot=T){
  ...
  if(isTRUE(plot)){
     walk(rownames(ph_assumption_test$table)[1:nrow(ph_assumption_test$table)-1], ~ggcoxzph(ph_assumption_test, var=.) %>% ggsave(filename=str_c(plot_output_path, '/ss_',.)))
  }
...
ph_assumption_paths <- (expand_grid(surv_ages, pp) %>% mutate(path = str_c('./OUTPUT/UKB/PLOTS/4_SURVIVAL/2_COXPH/ASS/', surv_ages, pp, sep='/')))$path
...
knitr::kable(multivariate_cox_tb2)

# print(ggcoxdiagnostics(multivariate_cox_interactions, type = "dfbeta",
...
# martingale_plotter <- function(input_tb, var){
# 
...
ggcoxfunctional(Surv(lastfollowup_age,status)~diagnosis_age + log(diagnosis_age)+I(sqrt(diagnosis_age)) + I(diagnosis_age^2), data=filter(ukb_tb,!is.na(diagnosis_age), diagnosis_age!=0))
ggcoxfunctional(Surv(lastfollowup_age,status)~bmi + log(bmi)+I(sqrt(bmi)) + I(bmi^2), data=filter(ukb_tb,!is.na(bmi)))
ggcoxfunctional(Surv(lastfollowup_age,status)~sbp + log(sbp)+I(sqrt(sbp))+ I(sbp^2), data=filter(ukb_tb,!is.na(sbp)))
ggcoxfunctional(Surv(lastfollowup_age,status)~cv_prs + I(cv_prs^2), data=filter(ukb_tb,!is.na(cv_prs)))
```
