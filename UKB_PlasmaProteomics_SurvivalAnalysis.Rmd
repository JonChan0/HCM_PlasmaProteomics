...
if (pp_scaling == 'scale'){
  pp <- ukb_pp_hcm_cov2 %>% select('nppb':'npc2') %>%
    scale()
...
ukb_tb <- contvar_binner(ukb_tb, 'ntprobnp', signif(quantile(ukb_tb$ntprobnp,probs=0.75,na.rm=T),3)) %>%
  ...
  contvar_binner('ace2',signif(quantile(ukb_tb$ace2,probs=0.75,na.rm=T),3))
```
...
# km_survival_curv_plotter(ukb_tb,ace2_categorical,overallcomp_status,overallcomp_age, plotpaths[3]) #Using top 25% vs. bottom 75% of ACE2
...
# km_survival_curv_plotter(ukb_tb,ace2_categorical,hfcomp_status,hfcomp_age, plotpaths[2])
...
# km_survival_curv_plotter(ukb_tb,ace2_categorical,vacomp_status,vacomp_age, plotpaths[1])
...
multivariate_coxph_plotter <- function(multivariate_results, plot_output_path, output_name, selected_vars = c('ntprobnp','hrc','ace2'),FDR=5, xlim=c(0,8), ylim=c(0,7), pp_scaling =F){
...
multivariate_coxph_plotter(multivariate_cox_results,path, 'ntprobnp_alloutcomes_fdr5', selected_vars=c('ntprobnp'), pp_scaling = pp_scaling)
multivariate_coxph_plotter(multivariate_cox_results,path, 'hrc_alloutcomes_fdr5', selected_vars=c('hrc'), pp_scaling = pp_scaling)
multivariate_coxph_plotter(multivariate_cox_results,path, 'allpp_alloutcomes_fdr5', pp_scaling = pp_scaling)
multivariate_coxph_plotter(multivariate_cox_results, path, 'allpp_alloutcomes_bonferroni', FDR=F, ylim=c(0,8), pp_scaling = pp_scaling)
