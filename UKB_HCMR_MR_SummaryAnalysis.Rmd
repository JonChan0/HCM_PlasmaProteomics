...
```{r pp_of_choice}
ukb_pps <- c(
  'NTproBNP','NPPB','TNNI3','HRC','ANGPT2','LTBP2','STC2','ACE2','F7','SHISA5'
)
hcmr_pps <- c('NTproBNP','TnTStat')
...
mr_results_or <- bind_rows(mr_results_or, filter(mr_results, method=='Wald ratio'))
...
forest_plot1 <- ggplot(sig_mr_results_or)+
  geom_point(aes(x=b, y= exposure_outcome, col=dataset))+
  geom_errorbar(aes(xmin=lo_ci, xmax=up_ci, y=exposure_outcome,col=dataset))+
  geom_vline(xintercept=0, col='black', linetype='dashed')+
  xlab('Change in Continuous Outcome (SD) per Log-Odds (HCM) Exposure')+
  ylab('Exposure -> Outcome')+
  labs(col='Dataset')

print(forest_plot1)
ggsave(str_c(results_folder,'sig_MR_forestplot.png'),forest_plot1, dpi=600, width=9, height=4.5)
```
