...
# rm(entity_field_tb)
```
Repeat for Olink's inflammation panel.

```{r}
include_inflam_panel=F

if (isTRUE(include_inflam_panel)){
  olink_inflam_panel <- read_csv2('DATA/UKB/RAP/Olink Explore Inflammation - 2024-09-24.csv') %>%
  mutate(ukb_fieldname_equivalent = str_to_lower(Gene))

  olink_inflam2_panel <- read_csv2('DATA/UKB/RAP/Olink Explore Inflammation II - 2024-09-24.csv') %>%
    mutate(ukb_fieldname_equivalent = str_to_lower(Gene))
  
  olink_inflam_panel <- bind_rows(olink_inflam_panel, olink_inflam2_panel)
  
  #Fix nomenclature differences 
  olink_inflam_panel$ukb_fieldname_equivalent[olink_inflam_panel$Gene=='HLA-DRA'] <- 'hla_dra'
  olink_inflam_panel$ukb_fieldname_equivalent[olink_inflam_panel$Gene=='HLA-E'] <- 'hla_e'
  
  olink_inflam_panel$ukb_fieldname_equivalent[olink_inflam_panel$Gene=='MICA_MICB'] <- 'micb_mica'
  olink_inflam_panel$Gene[olink_inflam_panel$ukb_fieldname_equivalent=='micb_mica'] <- 'MICB_MICA'
  
  olink_inflam_panel$ukb_fieldname_equivalent[olink_inflam_panel$Gene=='DEFB103A'] <- 'defb103a_defb103b'
  olink_inflam_panel$Gene[olink_inflam_panel$ukb_fieldname_equivalent=='defb103a_defb103b'] <- 'DEFB103A_DEFB103B'
  
  olink_inflam_panel$ukb_fieldname_equivalent[olink_inflam_panel$Gene=='CGB3'] <- 'cgb3_cgb5_cgb8'
  olink_inflam_panel$Gene[olink_inflam_panel$ukb_fieldname_equivalent=='cgb3_cgb5_cgb8'] <- 'CGB3_CGB5_CGB8'
  
  olink_inflam_panel$ukb_fieldname_equivalent[olink_inflam_panel$Gene=='AMY1A'] <- 'amy1a_amy1b_amy1c'
  olink_inflam_panel$Gene[olink_inflam_panel$ukb_fieldname_equivalent=='amy1a_amy1b_amy1c'] <- 'AMY1A_AMY1B_AMY1C'
  
  sum(!olink_inflam_panel$ukb_fieldname_equivalent %in% colnames(ukb_proteomics_data))
  olink_inflam_panel$ukb_fieldname_equivalent[!olink_inflam_panel$ukb_fieldname_equivalent %in% colnames(ukb_proteomics_data)]
  
  #Remove tnfsf9 which is not present in the UKB data
  olink_inflam_panel <- filter(olink_inflam_panel, Gene !='TNFSF9')
  sum(!olink_inflam_panel$ukb_fieldname_equivalent %in% colnames(ukb_proteomics_data))
  
  rm(olink_inflam2_panel)
}
...
if(instance =='1_I0'){
  ...
  #T2D cases -UKB prior to instance 0
  ...
  #HF Cases - UKB 5Y
  hf_eid <- read_csv('DATA/UKB/PROCESSED/2_5Y/ukb_rap_HF_20240209_5Ycases.tsv')
  #T2D cases -UKB 5Y
  t2d_cases <- read_tsv('DATA/UKB/PROCESSED/2_5Y/ukb_rap_T2D_20240209_5Ycases.tsv')
...
if (isTRUE(include_inflam_panel)){
  ...
  select(eid, instance, olink_cm_panel$ukb_fieldname_equivalent, olink_inflam_panel$ukb_fieldname_equivalent)
} else{
  ukb_pp_cmpanel <- ukb_proteomics_data %>%
...
#Add T2D status (depending on the instance 0 or 5Y or Total)
ukb_20240209_snptest_covariates <- ukb_20240209_snptest_covariates %>%
...
ukb_pp_hcm_cov2 <- ukb_pp_cmpanel %>%
  ...
  filter(if_any(nppb:npc2, ~!is.na(.))) #Filter out individuals who have no non-NA values for CM panel proteins = 52092 individuals

  #Write out the pp + covariates + HCM label depending on the time instancing
  if(isTRUE(include_inflam_panel)){
    write_tsv(ukb_pp_hcm_cov2, str_c('data/UKB/PROCESSED/', instance, '/ukb_cm_inflampanel_pp_hcm_covariates_bp_t2d_smoking.tsv'))
  } else{
...
```{r}
if (isTRUE(include_inflam_panel)){
  pp <- c(olink_cm_panel$ukb_fieldname_equivalent, olink_inflam_panel$ukb_fieldname_equivalent)
...
```{r}
summary_plotter_limma <- function(input_tb, output_label='ukb_cmpanel', predictor = 'HCM status', output_path=str_c('PLOTS/UKB/1_Case_vs_Control/2_Limma/',instance), pval='bonferroni', fdr_threshold = 0.05, test='Empirical Bayes moderated t-test',xlabel='Log2(Fold-Change)', xlimits=c(-1,1.75), specific_label=F){
  ...
  summary_cont_plot <- ggplot(input_tb, aes(x=logFC, y=-log10(P.Value), col=MTC_significant))+
    ...
    geom_text_repel(aes(label=Label), nudge_y=15, max.overlaps=40)+
    labs(col='Phenotype',shape='Predictor')+
...
#Running by default with age/sex/genetic PCs/dbp/T2D/smoking status covariates but comparing 103 HCM cases to non-HF/HCM controls
limma_hcm_2<- map(pp, ~limma_dpe(ukb_pp_hcm_cov,., covar=covars)) %>% bind_rows()

if(isTRUE(include_inflam_panel)){
  path <- str_c('./OUTPUT/UKB/PLOTS/1_Case_vs_Control/2_Limma/',instance,'/HCM_V2/cm_inflam/') #Define the folder for output of plots and tabular data
} else if (isTRUE(rcmm_cov)){
   path <- str_c('./OUTPUT/UKB/PLOTS/1_Case_vs_Control/2_Limma/',instance,'/HCM_V2/rcmm_cov/') #Define the folder for output of plots and tabular data
}else{
  path <- str_c('./OUTPUT/UKB/PLOTS/1_Case_vs_Control/2_Limma/',instance,'/HCM_V2/') #Define the folder for output of plots and tabular data
}


write_tsv(limma_hcm_2, str_c(path,'limma_results.tsv'))

if(isTRUE(include_inflam_panel)){
  limma_hcm_2 <- left_join(limma_hcm_2, bind_rows(olink_cm_panel, olink_inflam_panel) %>% select(Gene, `Protein name`, ukb_fieldname_equivalent), by=c('pp'='ukb_fieldname_equivalent'))
} else{
...
pp_vs_hcm_2_marginal_bonferroni_summarytb_limma <- summary_plotter_limma(limma_hcm_2, 'allpp_vs_hcm_2',predictor = 'HCM status',output_path=path) 

#Also for 5%FDR and 1%FDR
pp_vs_hcm_2_marginal_fdr5_summarytb_limma <- summary_plotter_limma(limma_hcm_2, 'allpp_vs_hcm_2',predictor = 'HCM status',output_path=path, pval = 'fdr') 
pp_vs_hcm_2_marginal_fdr1_summarytb_limma <- summary_plotter_limma(limma_hcm_2, 'allpp_vs_hcm_fdr1',predictor = 'HCM status',output_path=path, pval = 'fdr', fdr_threshold=0.01) 
...
pp_of_interest <- c('NTproBNP', 'NPPB','HRC','TNNI3','LTBP2','F7','EDN1','ACE2','CCL14') #5Y - 5% FDR
# pp_of_interest <- c('NTproBNP', 'NPPB','HRC','TNNI3','LTBP2','EDN1','ACE2','SHISA5','FABP3','F7','APOM') #OVERALL - Bonferroni

pp_of_interest <- filter(olink_cm_panel, Gene %in% pp_of_interest)$ukb_fieldname_equivalent
```
...
if (isTRUE(rcmm_cov)){
  path <- str_c('./OUTPUT/UKB/PLOTS/1_Case_vs_Control/2_Limma/',instance,'/HCM_V2/rcmm_cov/exclHFcases/') #Define the folder for output of plots and tabular data
} else {
  path <- str_c('./OUTPUT/UKB/PLOTS/1_Case_vs_Control/2_Limma/',instance,'/HCM_V2/exclHFcases/') #Define the folder for output of plots and tabular data
  
}

write_tsv(limma_hcm_exclHF, str_c(path,'cmpanel_limma_results.tsv'))
...
#Plot summary plots
pp_vs_hcm_exclHF_marginal_bonferroni_summarytb_limma <- summary_plotter_limma(limma_hcm_exclHF, 'allpp_vs_hcm_exclHF',predictor = 'HCM status',output_path=path
                                                                              #,xlimits=c(-1,2)
                                                                              ) 
pp_vs_hcm_exclHF_marginal_fdr5_summarytb_limma <- summary_plotter_limma(limma_hcm_exclHF, 'allpp_vs_hcm_exclHF',predictor = 'HCM status',output_path=path, pval='fdr'
                                                                        #,xlimits=c(-1,2)
                                                                        ) 
...
#Rerun Limma
limma_hcm_onlyHF <- map(olink_cm_panel$ukb_fieldname_equivalent, ~limma_dpe(ukb_pp_hcm_cov_onlyHF,.)) %>% bind_rows()

path <- str_c('./OUTPUT/UKB/PLOTS/1_Case_vs_Control/2_Limma/',instance,'/HCM_V2/onlyHFcases/') #Define the folder for output of plots and tabular data
...
if (isTRUE(rcmm_cov)){
  path <- str_c('./OUTPUT/UKB/PLOTS/1_Case_vs_Control/2_Limma/',instance,'/HF/rcmm_cov/') #Define the folder for output of plots and tabular data
} else{
  path <- str_c('./OUTPUT/UKB/PLOTS/1_Case_vs_Control/2_Limma/',instance,'/HF/') #Define the folder for output of plots and tabular data
}

write_tsv(hf_limma_results, str_c(path,'cmpanel_limma_results.tsv'))
...
summary_plotter_limma(hf_limma_results, 'allpp_vs_hf',predictor = 'HF status',output_path=path) 
summary_plotter_limma(hf_limma_results, 'allpp_vs_hf',predictor = 'HF status',output_path=path,pval='fdr') 

#Plotting only on the pp_of_interest
hf_pp_vs_hcm_marginal_bonferroni_summarytb_limma <- summary_plotter_limma(hf_limma_results %>% mutate(Label = ifelse(pp %in% pp_of_interest, Gene,'')), 'selectpp_vs_hf',predictor = 'HF status',output_path=path, specific_label=T) 
hf_pp_vs_hcm_marginal_fdr5_summarytb_limma <- summary_plotter_limma(hf_limma_results %>% mutate(Label = ifelse(pp %in% pp_of_interest, Gene,'')), 'selectpp_vs_hf',predictor = 'HF status',output_path=path,pval='fdr', specific_label=T) 
...
This is for online applications e.g ShinyGO.
...
write_tsv(select(filter(olink_cm_panel, Gene %in% filter(pp_vs_hcm_2_marginal_bonferroni_summarytb_limma, MTC_significant==T)$Gene),`UniProt ID`),
          str_c('./OUTPUT/UKB/PLOTS/1_Case_vs_Control/2_Limma/',instance,'/HCM_V2/',gap,'bonferroni_pass_uniprotIDs.tsv'), col_names=F)
...
write_tsv(select(filter(olink_cm_panel, Gene %in% filter(pp_vs_hcm_2_marginal_fdr1_summarytb_limma, MTC_significant==T)$Gene),`UniProt ID`),
          str_c('./OUTPUT/UKB/PLOTS/1_Case_vs_Control/2_Limma/',instance,'/HCM_V2/',gap,'fdr1_pass_uniprotIDs.tsv'), col_names=F)
...
write_tsv(select(filter(olink_cm_panel, Gene %in% filter(pp_vs_hcm_2_marginal_fdr5_summarytb_limma, MTC_significant==T)$Gene),`UniProt ID`),
          str_c('./OUTPUT/UKB/PLOTS/1_Case_vs_Control/2_Limma/',instance,'/HCM_V2/',gap,'fdr5_pass_uniprotIDs.tsv'), col_names=F)
...
limma_results <- read_tsv('OUTPUT/UKB/PLOTS/1_Case_vs_Control/2_Limma/2_5Y/HCM_V2/cmpanel_limma_results.tsv')
fdr5_pass <- read_tsv('OUTPUT/UKB/PLOTS/1_Case_vs_Control/2_Limma/2_5Y/HCM_V2/fdr5_pass_uniprotIDs.tsv', col_names=F)
background_uniprot <- read_tsv('DATA/UKB/RAP/olink_cmpanel_genes.tsv',col_names=F)
...
if(isTRUE(rcmm_cov)){
  path <- str_c('OUTPUT/UKB/PLOTS/3_GO/',instance,'/rcmm_cov/')
} else{
  path <- str_c('OUTPUT/UKB/PLOTS/3_GO/',instance,'/')
}
...
I also perform Cox regression to evaluate associations of plasma proteins with **incident** diagnosis of HCM in all of UKB-PPP (excluding baseline instance 0 cases of HCM).
...
if(isTRUE(rcmm_cov)){
  ukb_pp <- read_tsv(str_c('DATA/UKB/PROCESSED/',instance,'/ukb_cmpanelpp_hcm_covariates_bp_t2d_smoking_rcmmcov.tsv')) %>%  #This shouldn't matter as all of the pp values should be the same across all
  select(-hcm)
  
} else{
  ukb_pp <- read_tsv(str_c('DATA/UKB/PROCESSED/',instance,'/ukb_cmpanelpp_hcm_covariates_bp_t2d_smoking.tsv')) %>%  #This shouldn't matter as all of the pp values should be the same across all
  select(-hcm)
}
...
multivariate_cox_results <- c(
  map(olink_cm_panel$ukb_fieldname_equivalent, ~multivariate_cox(ukb_diag_cox_tb,.,covariates,'incidenthcm_status','incidenthcm_age', outcome_levels=c('INCIDENTHCM'), print_summary = F))
) %>%
  bind_rows() %>%
  filter(!is.na(p.value))

multivariate_cox_results <- multivariate_cox_results %>%
  left_join(select(olink_cm_panel,ukb_fieldname_equivalent,Gene), by=c('var'='ukb_fieldname_equivalent')) %>%
  dplyr::rename(Label=Gene)

if(isTRUE(rcmm_cov)){
  saveRDS(multivariate_cox_results, str_c('OUTPUT/UKB/PLOTS/5_CoxHCMDiag/',instance,'/rcmm_cov/allcmpp_mvCox_results.rds'))
} else{
  saveRDS(multivariate_cox_results, str_c('OUTPUT/UKB/PLOTS/5_CoxHCMDiag/',instance,'/allcmpp_mvCox_results.rds'))
}
...
```{r}
multivariate_coxph_plotter <- function(multivariate_results, plot_output_path, output_name, FDR=5, specific_label=F){
...
```{r}
if(isTRUE(rcmm_cov)){
  path <- str_c('./OUTPUT/UKB/PLOTS/5_CoxHCMDiag/', instance,'/rcmm_cov/')
}else{
  path <- str_c('./OUTPUT/UKB/PLOTS/5_CoxHCMDiag/', instance,'/')
}

mvcox_incidentHCM_fdr5 <- multivariate_coxph_plotter(multivariate_cox_results, path, 'allCMpp_incidentHCMdiag_fdr5')
multivariate_coxph_plotter(multivariate_cox_results, path, 'allCMpp_incidentHCMdiag_fdr1', FDR=1)
multivariate_coxph_plotter(multivariate_cox_results, path, 'allCMpp_incidentHCMdiag_bonferroni', FDR=F)

#Write out 
write_tsv(select(filter(olink_cm_panel, ukb_fieldname_equivalent %in% filter(mvcox_incidentHCM_fdr5, MTC_significant==T)$var),`UniProt ID`),
          str_c(path,'fdr5_pass_uniprotIDs.tsv'), col_names=F)
