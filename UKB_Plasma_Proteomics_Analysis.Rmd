...
dist_plotter <- function(pheno, input_tb, output_folder='OUTPUT/UKB/PLOTS/0_EDA/3_HCM_SUBSET_COMP/'){
  ...
  wilcox_result<- wilcox.test(filter(input_tb,hcm_instance=='<5Y Case')[[pheno]], filter(input_tb, hcm_instance=='>5Y Case')[[pheno]], conf.int=T)
  
  p_value <- wilcox_result$p.value

  # Extract confidence interval
  conf_int <- wilcox_result$conf.int
  
  # Print results
  print(paste("P-value:", p_value))
  print(paste("Confidence Interval:", conf_int[1], "to", conf_int[2]))
  ...
  density_plot <- ggplot(input_tb, aes(x=eval(parse(text=pheno))))+
    ...
    labs(caption = str_c('Wilcoxon Test between Case Subgroups: p-value = ', signif(p_value,digits=3), ' and 95% CI = [',signif(conf_int[1],digits=3), ',',signif(conf_int[2],digits=3),']'))+
...
summary_plotter_limma <- function(input_tb, output_label='ukb_cmpanel', predictor = 'HCM status', output_path=str_c('PLOTS/UKB/1_Case_vs_Control/2_Limma/',instance), pval='bonferroni', fdr_threshold = 0.05, test='Empirical Bayes moderated t-test',xlabel='Log2(Fold-Change)', xlimits=c(-1,1.75), specific_label=F){
  ...
  summary_cont_plot <- ggplot(input_tb, aes(x=logFC, y=-log10(P.Value), col=MTC_significant))+
    ...
    geom_text_repel(aes(label=Label), nudge_y=15, max.overlaps=100)+
    labs(col='Phenotype',shape='Predictor')+
...
path <- str_c('./OUTPUT/UKB/PLOTS/1_Case_vs_Control/2_Limma/',instance,'/HCM_V2/', extra_folders) #Define the folder for output of plots and tabular data

write_tsv(limma_hcm_2, str_c(path,'limma_results.tsv'))
limma_hcm_2 <- left_join(limma_hcm_2, select(ukb_pp_list, Gene=Target, ukb_fieldname_equivalent), by=c('pp'='ukb_fieldname_equivalent')) %>% unique()
...
path <- str_c('./OUTPUT/UKB/PLOTS/1_Case_vs_Control/2_Limma/',instance,'/HCM_V2/exclHFcases/', extra_folders) #Define the folder for output of plots and tabular data
...
write_tsv(select(filter(ukb_pp_list, Target %in% filter(pp_vs_hcm_2_marginal_bonferroni_summarytb_limma, MTC_significant==T)$Gene),UniProt),
          str_c('./OUTPUT/UKB/PLOTS/1_Case_vs_Control/2_Limma/',instance,'/HCM_V2/',extra_folders,'bonferroni_pass_uniprotIDs.tsv'), col_names=F)

write_tsv(select(filter(ukb_pp_list, Target %in% filter(pp_vs_hcm_2_marginal_fdr1_summarytb_limma, MTC_significant==T)$Gene),UniProt),
          str_c('./OUTPUT/UKB/PLOTS/1_Case_vs_Control/2_Limma/',instance,'/HCM_V2/',extra_folders,'fdr1_pass_uniprotIDs.tsv'), col_names=F)

write_tsv(select(filter(ukb_pp_list, Target %in% filter(pp_vs_hcm_2_marginal_fdr5_summarytb_limma, MTC_significant==T)$Gene),UniProt),
          str_c('./OUTPUT/UKB/PLOTS/1_Case_vs_Control/2_Limma/',instance,'/HCM_V2/',extra_folders,'fdr5_pass_uniprotIDs.tsv'), col_names=F)
...
limma_results <- read_tsv(str_c('OUTPUT/UKB/PLOTS/1_Case_vs_Control/2_Limma/',instance,'/HCM_V2/',extra_folders,'/limma_results.tsv'))
fdr5_pass <- read_tsv(str_c('./OUTPUT/UKB/PLOTS/1_Case_vs_Control/2_Limma/',instance,'/HCM_V2/',extra_folders,'fdr5_pass_uniprotIDs.tsv'), col_names=F)
...
```{r}
multivariate_coxph_plotter <- function(multivariate_results, plot_output_path, output_name, pp_scaling, FDR=5, specific_label=F){
