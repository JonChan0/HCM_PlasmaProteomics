...
hcmr_rarevar <- read_tsv('rarevar_subset_analysis/output/data/hcmr_rarevar_class_plp_vusfpvus.tsv') %>%
  dplyr::rename('rarevar_class'=class, 'HCR_IDs'='HCR_ID')
  
total_df_notransform <- readRDS('output/data/total_notransform_df.rds') %>%
  mutate(ecvfwhole=ecvfwhole*100)

total_i_all_cont_phenotypes <- readRDS('output/data/total_i_all_cont_phenotypes.rds')
...
label_tb <- read_tsv('output/data/combo_labels.tsv') 
...
```{r normalisation}
#Inverse based rank transform all the phenotypes
...
summary_plotter <- function(input_tb,output_label, output_path='output/2_CrossPheno_AssocAnalysis/hcmr/summary_plots/', pval='bonferroni',confint=F, se=F,test='t-Test', xlabel='Change in Phenotype per Unit Increase in Predictor'){
...
write_tsv(all_vs_pp_marginal,'output/2_CrossPheno_AssocAnalysis/hcmr/all_vs_pp_marginal.tsv')
...
```{r}
summary_plotter_individualpredictor <- function(input_tb,predictor_of_interest, output_path='output/2_CrossPheno_AssocAnalysis/hcmr/summary_plots/', pval='bonferroni',confint=F, se=F,test='t-Test', xlabel='Change in Phenotype per Unit Increase in Predictor', ylimits='', xlimits=''){
...
lvef_stratifier_check <- function(input_tb, pheno_of_interest,lvef_cutoff=50, plot_output_path = 'output/2_CrossPheno_AssocAnalysis/hcmr/misc/', suffix=''){
