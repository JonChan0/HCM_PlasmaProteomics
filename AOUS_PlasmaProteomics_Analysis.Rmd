...
genetic_ancest <- bucket_rds_importer(str_c(bucket_basepath, 'all_geneticancest.rds'),'all_geneticancest.rds')
...
system('gsutil -m cp -r ~/rstudio/PP/DATA/ gs://fc-secure-7953e92c-a6a6-42df-9f19-86d553a9044f/PP/' )
system('gsutil -m cp -r ~/rstudio/PP/OUTPUT/ gs://fc-secure-7953e92c-a6a6-42df-9f19-86d553a9044f/PP/' )
...
age_sex_hcm_cases_controls <- regenie_cov2 %>% select(IID, sex, age) %>%
  mutate(hcm = ifelse(IID %in% hcm_cases$IID, T,F)) %>%
...
matched_list <- map(c(40,100,200), ~age_sex_matcher(age_sex_hcm_cases_controls, case_control_ratio=.))

print(summary(matched_list[[3]]))

# plot(matched_list[[3]], type = "jitter", interactive = FALSE)
```

Now extracting the matched controls to the cases for a ratio of 200 (because it performs well)

```{r}
matched_cases_controls <- match.data(matched_list[[3]])
...
write_tsv(matched_regenie_pheno, 'hcm_regenie_pheno_agesexmatched200.tsv')
write_tsv(matched_regenie_cov, 'hcm_regenie_cov_agesexmatched200.tsv')

system('gsutil cp hcm_regenie_pheno_agesexmatched200.tsv hcm_regenie_cov_agesexmatched200.tsv -t gs://fc-secure-7953e92c-a6a6-42df-9f19-86d553a9044f/PP/DATA/' )
