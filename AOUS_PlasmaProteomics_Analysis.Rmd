...
zip_socioeconomic_observations <- read_bq_export_from_workspace_bucket('gs://fc-secure-7953e92c-a6a6-42df-9f19-86d553a9044f/bq_exports/jon126@researchallofus.org/20240925/zip_code_socioeconomic_18092247/zip_code_socioeconomic_18092247_*.csv') %>%
  select(person_id, observation_datetime, deprivation_index)
...
biomarker_cleaner <- function(input_tb, biomarker_of_interest, colname_of_interest, upper_limit, chosen_conc_unit='ng/mL',molar_mass=NA ){#For creatinine
  ...
  output_tb <- harmonised_tb %>%
    ...
    filter(value_as_number < upper_limit) #Filter only for values below the upper limit
...
upper_limits <- c(200,1000,5000,1000,1000) #This defines the upper limit for cutoff in ng/mL for each biomarker based on EDA of the transformed values
...
rm(demographics_cleaned, bmi_cleaned, dbp_cleaned)
```
...
saveRDS(demo_cov, 'DATA/all_demo_bmi_dbp.rds')
...
walk(cardio_biomarkers_cleaned, ~print(length(unique(.[['person_id']])))) #NPPB/NTproBNP/TnI/TnT/BAP
```
...
walk(cardio_biomarkers_cleaned, ~single_col_dist_plotter(., 3, label_tb, save=T))
...
names(diagnoses_cleaned) <- condition_names
biomarker_diagnoses_list <- map(cardio_biomarkers_cleaned, ~biomarker_diagnoses_formatter(.,diagnoses_cleaned))
saveRDS(biomarker_diagnoses_list, 'DATA/biomarker_list_hcm_hf_t2d_statuses_i0_5Y_overall.rds')

#biomarker_diagnoses_list <- readRDS('DATA/biomarker_list_hcm_hf_t2d_statuses_i0_5Y_overall.rds')
```
...
demo_covar_extractor <- function(input_tb, demo_cov, biomarker_name){
  ...
  print(sum(!is.na(output_tb$race) & !is.na(output_tb$bmi) & !is.na(output_tb$dbp)))
...
biomarker_diagnoses_demo2_lst <- readRDS('DATA/biomarker_diagnoses_demo2_lst.rds')
```
...
walk(biomarker_diagnoses_demo2_lst, ~nonNA_checker(., c('age','sex_at_birth','bmi','pc1','pc2','pc3','pc4','pc5','dbp')))
#For each of the biomarkers in biomarker_names
```
...
```{r}
hcm_all_vs_nonhcmhf <- function(input_tb, biomarker, instance='OV'){ #This runs for each biomarker at a time
...
hcm_all_vs_nonhcmhf_ov <- map2(biomarker_diagnoses_demo2_lst, biomarker_names, ~hcm_all_vs_nonhcmhf(.x, .y)) %>% bind_rows()
hcm_all_vs_nonhcmhf_i0 <- map2(biomarker_diagnoses_demo2_lst, biomarker_names, ~hcm_all_vs_nonhcmhf(.x, .y, instance='i0')) %>% bind_rows()
hcm_all_vs_nonhcmhf_5Y <- map2(biomarker_diagnoses_demo2_lst, biomarker_names, ~hcm_all_vs_nonhcmhf(.x, .y, instance='5Y')) %>% bind_rows()
...
saveRDS(hcm_all_vs_nonhcmhf_results, 'OUTPUT/DATA/hcm_all_vs_nonhcmhf.rds')
```
...
```{r}
hcm_nonhf_vs_nonhcmhf <- function(input_tb, biomarker, instance='OV'){ #This runs for each biomarker at a time
...
hcm_nonhf_vs_nonhcmhf_ov <- map2(biomarker_diagnoses_demo2_lst, biomarker_names, ~hcm_nonhf_vs_nonhcmhf(.x, .y)) %>% bind_rows()
hcm_nonhf_vs_nonhcmhf_i0 <- map2(biomarker_diagnoses_demo2_lst, biomarker_names, ~hcm_nonhf_vs_nonhcmhf(.x, .y, instance='i0')) %>% bind_rows()
hcm_nonhf_vs_nonhcmhf_5Y <- map2(biomarker_diagnoses_demo2_lst, biomarker_names, ~hcm_nonhf_vs_nonhcmhf(.x, .y, instance='5Y')) %>% bind_rows()
...
saveRDS(hcm_nonhf_vs_nonhcmhf_results , 'OUTPUT/DATA/hcm_nonhf_vs_nonhcmhf.rds')
#Plot for each instance
...
```{r}
hcm_hf_vs_nonhcmhf <- function(input_tb, biomarker, instance='OV'){ #This runs for each biomarker at a time
...
hcm_hf_vs_nonhcmhf_ov <- map2(biomarker_diagnoses_demo2_lst, biomarker_names, ~hcm_hf_vs_nonhcmhf(.x, .y)) %>% bind_rows()
hcm_hf_vs_nonhcmhf_i0 <- map2(biomarker_diagnoses_demo2_lst, biomarker_names, ~hcm_hf_vs_nonhcmhf(.x, .y, instance='i0')) %>% bind_rows()
hcm_hf_vs_nonhcmhf_5Y <- map2(biomarker_diagnoses_demo2_lst, biomarker_names, ~hcm_f_vs_nonhcmhf(.x, .y, instance='5Y')) %>% bind_rows()
...
saveRDS(hcm_hf_vs_nonhcmhf_results , 'OUTPUT/DATA/hcm_hf_vs_nonhcmhf.rds')
#Plot for each instance
...
```{r}
nonhcm_hf_vs_nonhcmhf <- function(input_tb, biomarker, instance='OV'){ #This runs for each biomarker at a time
...
nonhcm_hf_vs_nonhcmhf_ov <- map2(biomarker_diagnoses_demo2_lst, biomarker_names, ~nonhcm_hf_vs_nonhcmhf(.x, .y)) %>% bind_rows()
nonhcm_hf_vs_nonhcmhf_i0 <- map2(biomarker_diagnoses_demo2_lst, biomarker_names, ~nonhcm_hf_vs_nonhcmhf(.x, .y, instance='i0')) %>% bind_rows()
nonhcm_hf_vs_nonhcmhf_5Y <- map2(biomarker_diagnoses_demo2_lst, biomarker_names, ~nonhcm_f_vs_nonhcmhf(.x, .y, instance='5Y')) %>% bind_rows()
...
saveRDS(nonhcm_hf_vs_nonhcmhf_results , 'OUTPUT/DATA/nonhcm_hf_vs_nonhcmhf.rds')
...
regenie_cov <- regenie_cov %>%
  mutate(FID=person_id, IID=person_id) %>%
...
system('gsutil cp -m -r ~/rstudio/PP/ gs://fc-secure-7953e92c-a6a6-42df-9f19-86d553a9044f/' )
