...
biomarker_cleaner <- function(input_tb, biomarker_of_interest, colname_of_interest, 
  ...
  output_tb <- harmonised_tb %>%
    ...
    # filter(value_as_number < upper_limit) %>% #Deprecated filtering
...
# upper_limits <- c(200,1000,5000,1000
                  #,1000
                  # ) #This defines the upper limit for cutoff in ng/mL for each biomarker based on EDA of the transformed values = DEPRECATED as of 23/12/2024 due to implementation of 3SD cutoff instead

cardio_biomarkers_cleaned <- pmap(list(biomarkers, biomarker_names), ~biomarker_cleaner(cardio_biomarkers, ..1,..2))
...
system('gsutil -m cp -r ~/PP/DATA/ gs://fc-secure-7953e92c-a6a6-42df-9f19-86d553a9044f/PP/' )
system('gsutil -m cp -r ~/PP/OUTPUT/ gs://fc-secure-7953e92c-a6a6-42df-9f19-86d553a9044f/PP/' )
